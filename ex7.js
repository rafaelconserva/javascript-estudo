...
const moniqueSacola = [
  ...
  { fruta: "laranja", preco: 20.0, quantidade: 10 }
];
...
console.log("================= CARRINHO UNIFICADO =================");

const carrinho = dimitriSacola.concat(moniqueSacola);
...
carrinho.forEach(item => {
  console.log(
    "Produto: " + item.fruta + " Preço: R$ " + item.preco.toFixed(2) + " Quantidade: " + item.quantidade
  );
  totalCarrinho += item.preco * item.quantidade;
  quantidadeFrutas += item.quantidade;
});

console.log("Frutas adicionadas: " + quantidadeFrutas);
console.log("Total a ser pago: R$ " + totalCarrinho.toFixed(2));
